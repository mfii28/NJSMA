            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="content-card shadow-sm mb-4">
                        ...
                        <?php if (!empty($GLOBAL_SETTINGS['charter_intro'])): ?>
                            <p class="text-muted"><?= nl2br(htmlspecialchars($GLOBAL_SETTINGS['charter_intro'])) ?></p>
                        <?php else: ?>
                        ...
                        <?php if (!empty($GLOBAL_SETTINGS['charter_commitments'])): ?>
                            <ul class="list-unstyled" style="line-height: 2;">
                                <?php foreach (explode("\n", $GLOBAL_SETTINGS['charter_commitments']) as $commitment): ?>
                                    <?php if (trim($commitment)): ?>
                                        <li><i class="bi bi-check-circle-fill text-success me-2"></i> <?= htmlspecialchars(trim($commitment)) ?></li>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                            </ul>
                        <?php else: ?>
                ...
                <div class="col-lg-4">
                    <div class="info-banner shadow-sm">
                        ...
                        <div class="mt-3">
                            <p class="mb-2"><strong>Hotline:</strong> <?= htmlspecialchars($GLOBAL_SETTINGS['charter_hotline'] ?? '0342293669') ?></p>
                            <p class="mb-0"><strong>Email:</strong> <?= htmlspecialchars($GLOBAL_SETTINGS['charter_email'] ?? 'complaints@njsma.gov.gh') ?></p>
                        </div>
                    </div>
                </div>
            </div>
